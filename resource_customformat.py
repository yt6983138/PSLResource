...
def save(key, entry):
    ...
    if (config["avatar"] and key[:7] == "avatar."):
        ...
        bytesIO = BytesIO()
        obj.image.save(bytesIO, "png")
        queue_in.put((key, bytesIO))
    elif (key[-4:] == ".png"): # all other pngs
        if ("IllustrationLowRes.png" in key and not config["IllustrationLowRes"]): return
        if ("Illustration.png" in key and not config["Illustration"]): return
        pool.submit(save_image, key, obj.image)
...
def run(path, c):
    ...
    for x in range(reader.readInt()): # binary parsing
...
if __name__ == "__main__":
    ...
    types = c["TYPES"]
    #run("""G:/Large files/main.82.com.PigeonGames.Phigros/PhigrosLibrary_Resource/new/3.5/main.98.com.PigeonGames.Phigros.obb""", {
    run(sys.argv[1], {
